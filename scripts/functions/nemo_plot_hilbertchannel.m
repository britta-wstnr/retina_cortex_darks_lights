...
% https://github.com/meeg-cfin/nemolab/blob/master/hilbertsourceloc/nemo_plot_hilbertchannel.m

% NEMO_PLOT_HILBERTCHANNEL plots the output of nemo_hilberstats_sensorlevel
% as TFR of a specified parameter and one channel
%
% Use as
%       plot_hilbertchannel(cfg, data_tf)
%
% You should specify the following parameters:
%   cfg.funparameter       = parameter you want to plot, can be 'stat', 'avg',
%                            'itc', 'pval'
%   cfg.channel            = string specifying the channel that should be
%                            plotted, as specified in data_tf.label
%   cfg.freqbands          = n_freq x 2 matrix specifying the lower and upper
%                            limits of the frequency bands (see below!)
%   cfg.zlim               = limits for color dimension, can be 'maxabs'
%                            (default), 'mincenter' (centered at zero, minimum
%                            value is limit), 'maxcenter' or [min max]
%   cfg.xlim               = time limits for plotting
%   cfg.masking            = 0 or 1. If 1, masking is carried out, i.e. the
%                            binary mask in cfg.maskparameter will be used to
%                            only show the areas where the mask is 1
%   cfg.marking            = 0 or 1. If 1, the areas where the binary mask
%                            specified in cfg.maskparameter is 1 will be
%                            marked by a black rectangle, if the contain more
%                            data points than specified in cfg.markingclust
%   cfg.maskparameter      = if masking or marking is desired, this should
%                            point to a field in data_tf that contains a binary
%                            mask
%   cfg.markingclust       = number that specifies the minimum number of
%                            datapoints in a "cluster" (specified by the binary
%                            mask), that should be outlined by a rectangle
%   cfg.zeroline           = 0 or 1. If 1, a vertical line is drawn to indicate
%                            0 on the time axis.
%
...
if(iscell(cfg.channel))    % if channel is specified as a cell:
                           % need to check that is just one channel
...
if(chandim==1)   % is there a more elegant way to do this? is it even necessary
                 % cause data come from hilbertstats script most likely anyway?
...
if(isfield(cfg, 'maskparameter'))
    ...
    if(chandim==1)   % is there a more elegant way to do this? is it even
                     % necessary cause data come from hilbertstats script most
                     % likely anyway?!
...
% make a copy of the original freqbands (before we maybe mess with it due to
% ommitted freqs):
...
if(cfg.marking)
    ...
    for ii=1:size(dat,1)
        ...
        if(sum(maskcumlsum)==0)   % no significant timepoints at all
            ...
            countstart = 0; countstop = 0; % counter for start and stop of
                                           % clusters

            % STARTPOINTS OF CLUSTERS
            % first check the 1st sample:
            if(maskcumlsum(1)==1)   % if the cluster starts at the beginning of
                                    % the whole thing
            ...
            for n=2:length(maskcumlsum)-2 % we do not want to check the last
                                          % point here

                if(maskcumlsum(n) < maskcumlsum(n+1) && maskcumlsum(n)  == ...
                   maskcumlsum(n-1))
                    countstart = countstart + 1;
...
if(size(idx,2)~=0)      % if there are gaps in data
    ...
    for ii=1:size(idx,2)
        ...
        gapfreq = [data_tf.freqbands(idx(ii),2), ...
                   data_tf.freqbands(idx(ii)+1,1)];
        data_tf.freqbands = [data_tf.freqbands(1:idx(ii),:); gapfreq; ...
                            data_tf.freqbands(idx(ii)+1:end,:)];
...
if(isnumeric(cfg.zlim))   % if user specified own values
    ...
    if(isfield(cfg, 'xlim'))
        xmin = dsearchn(data_tf.time',cfg.xlim(1)); xmax = ...
               dsearchn(data_tf.time',cfg.xlim(2));
    else
    ...
    switch cfg.zlim
        case 'maxabs'
            ...
            clims = max(abs([cmin, cmax]));
            % automatically center zero on color bar
        case 'mincenter'
            clims = abs(min(min(datplot(:,xmin:xmax))));
            % centered, max values are min and +min
        case 'maxcenter'
            clims = abs(max(max(datplot(:,xmin:xmax))));
            % centered, max values are -max and max
    end
    climlo = -clims;
    climup = clims;
end
...
if(cfg.zeroline)
    hold on
        z = round(max(max(datplot))) + 5;   % just make sure that it is
...
if(cfg.marking)
    z = round(max(max(datplot))) + 5;   % just make sure that it is "on top"
                                        % of all the rest;
    hold on
    for ii=1:size(mask,1)
        for jj=1:size(clusterson{ii},2)
            plot3([clusterson{ii}(jj), clustersoff{ii}(jj), ...
                   clustersoff{ii}(jj), clusterson{ii}(jj), ...
                   clusterson{ii}(jj)], ...   % x axis
                [freqorig(ii, 1), freqorig(ii, 1),  freqorig(ii, 2), ...
                 freqorig(ii, 2), freqorig(ii, 1)], ...    % y axis
                [z, z, z, z, z], ...                                                                                            % z axis
                'color', [0,0,0], 'linewidth', 1.5); hold on
        end
    end
